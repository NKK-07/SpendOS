...
with open('apps/dashboard/src/app/tickets/page.tsx', 'r', encoding='utf-8') as f:
    code = f.read()

# Replace ResolveModal entirely and update TicketsPage
new_code = """"use client";

import { useEffect, useState } from 'react';
import { useApi, useAuth, isAdminUp } from '../auth';
...
export default function TicketsPage() {
  ...
  const load = () => {
    setLoading(true);
    api('/tickets').then(r => r.json()).then(d => setTickets(Array.isArray(d) ? d : [])).finally(() => setLoading(false));
  };
...
with open('apps/dashboard/src/app/tickets/page.tsx', 'w', encoding='utf-8') as f:
    f.write(new_code)
print("Tickets page updated.")
